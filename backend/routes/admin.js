...
// ── Telegram Config ──────────────────────────────────────────
// Hands the bot token + channel ID to the authenticated admin panel so it can
// upload files directly to Telegram from the browser (needed since some files
// exceed Vercel's serverless request body limit). Only reachable by someone
// who already passed the admin password check above.
...
router.get('/videos', async (req, res) => {
    try {
        ...
        const videos = result.rows.map(v => {
            if (v.telegram_thumb_id) v.thumbnail_url = `/api/videos/${v.id}/thumb`;
            return v;
        });
        res.json({ videos });
    } catch (err) {
...
router.post('/performers', async (req, res) => {
    try {
        ...
        if (err.code === '23505') return res.status(409).json({ error: 'Slug already exists.' });
        console.error('Performer create error:', err);
...
router.put('/performers/:id', async (req, res) => {
    try {
        ...
        console.error('Performer update error:', err);
...
router.delete('/performers/:id', async (req, res) => {
    try {
        ...
        console.error('Performer delete error:', err);
...
router.post('/galleries', async (req, res) => {
    try {
        const { title, category_id, orientation, performer_ids, tags, telegram_file_ids } = req.body;
        if (!title || !telegram_file_ids || !telegram_file_ids.length) {
            return res.status(400).json({ error: 'title and at least one image are required.' });
        }

        const result = await db.query(
            `INSERT INTO galleries (title, category_id, orientation)
             VALUES ($1,$2,$3) RETURNING id`,
            [title, category_id || null, orientation || 'straight']
        );
        const galleryId = result.rows[0].id;

        for (let i = 0; i < telegram_file_ids.length; i++) {
            await db.query(
                'INSERT INTO gallery_images (gallery_id, telegram_file_id, sort_order) VALUES ($1,$2,$3)',
                [galleryId, telegram_file_ids[i], i]
            );
        }
