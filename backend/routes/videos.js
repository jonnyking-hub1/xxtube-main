...
const { generateBunnyStreamUrl } = require('../services/bunny');
    ...
    try {
        ...
        const result = await db.query(
            `SELECT DISTINCT
                ...
                COALESCE(
                    (SELECT json_agg(DISTINCT jsonb_build_object('id', p.id, 'name', p.name, 'slug', p.slug))
...
router.get('/:id/related', async (req, res) => {
    try {
        ...
        const result = await db.query(
            `SELECT id, title, thumbnail_url, duration_seconds, price_euros, views_count, is_vr, is_amateur
             FROM videos
